...
class ModelAuth extends ModelBase 
{
	...
	public function setUp() {
		...
		$this->inspector->addValidator('validpassword', function($value, $inspector) {
			...
			if ($inspector instanceof \app\Inspector) {
				if ($inspector->hasProperty('validUser') && $inspector->validUser instanceof \app\Model\Orm\Users) {
	...
	/**
	 * Metode authentifikasi via Github
	 *
	 * @param array Github data 
	 * @param string Github access token 
	 *
	 * @return bool Sukses atau tidaknya proses
	 */
	public function loginGithub($githubData = array(), $accessToken = NULL) {
		// Inisialisasi
		$result = new Parameter(array('success' => false, 'error' => NULL));
		...
		if ( ! empty($username) || ! empty($email)) {
			...
			if ($validUser && ! empty($accessToken)) {
				...
				$parameter->set('github_access_token', $accessToken);
				ModelBase::factory('User')->updateUserData($validUser->getUid(),$parameter->all());
	...
	public function register($userData = array()) {
		...
		$this->inspector->setSubstance($userData);
		$this->inspector->ensure('username')->notEmpty('username cannot empty')
		                                    ->isMin(3, 'username too short')
		                                    ->notValidUsername('username already registered');
		$this->inspector->ensure('email')->notEmpty('email cannot empty')
		                                 ->isEmail('invalid email')
		                                 ->notValidUsername('email already registered');
		$this->inspector->ensure('password')->notEmpty('password cannot empty')
		                                    ->isSameAs('cpassword', 'password confirmation did not match');
	 ...
	 * @return mixed FALSE jika not found, Users object bila ditemukan
	 */
	public function isUser($identifier = NULL) {
		...
		// Cek email
		$user = ModelBase::ormFactory('UsersQuery')->findOneByMail($identifier);

		if (empty($user)) {
			// Cek nama
			$user = ModelBase::ormFactory('UsersQuery')->findOneByName($identifier);
		}
	 ...
	 * @return bool
	 */
	public function isValidPassword(Users $user, $password = '') {
		// Ambil stored hash
	...
	public function confirm($token) {
		$result = new Parameter(array('success' => false, 'error' => NULL));
		$relatedUser = ModelBase::ormFactory('UsersQuery')->findOneByPass($token);
	...
	public function sendConfirmation($uid = 0) {
		$validUser = ModelBase::ormFactory('UsersQuery')->findPK($uid);
	...
	public function sendReset($email) {
		$validUser = ModelBase::ormFactory('UsersQuery')->findOneByMail($email);
