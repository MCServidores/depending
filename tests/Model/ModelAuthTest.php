...
class ModelAuthTest extends DependingInTestCase {
	...
	public function testCekRegisterModelAuth() {
		...
		$data = array('username' => 'dummy', 'email' => 'frei.denken@Github.com', 'password' => 'secret', 'cpassword' => 'secret');
	...
	public function testCekKirimResetModelAuth() {
		...
		$auth->createUser('dummy', 'frei.denken@Github.com', 'secret');
	...
	/**
	 * Cek Login via Github
	 */
	public function testCekLoginGithubModelAuth() {
		$auth = new ModelAuth();
		$token = 'SomeToken';

		// Invalid data
		$data = array();
		$hasilLoginGithub = $auth->loginGithub($data,$token);

		$this->assertFalse($hasilLoginGithub->get('success'));
		...
		$this->assertTrue($hasilLoginGithub->get('success'));
	}
